...
class NewCommand extends Command {
	...
	protected function configure() {
		$this
			->setName( 'new' )
			->setDescription( 'Create a new PressGang theme' )
...
╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╰━━╯</>' . PHP_EOL . PHP_EOL );
		...
		$commands = [
			$composer . " create-project pressgang/pressgang-wp \"$directory\" $version --remove-vcs --prefer-dist",
		];
		...
		if ( ( $process = $this->runCommands( $commands, $input, $output ) )->isSuccessful() ) {
			...
			$output->writeln( PHP_EOL . '<comment>PressGang Child Theme is ready! Build something amazing.</comment>' );
		}
	...
	protected function createRepository( string $directory, InputInterface $input, OutputInterface $output ) {
		...
		$commands = [
			'git init -q',
			'git add .',
			'git commit -q -m "Set up a fresh PressGang theme"',
			"git branch -M {$branch}",
		];
